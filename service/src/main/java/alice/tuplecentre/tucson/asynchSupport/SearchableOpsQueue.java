...
public class SearchableOpsQueue extends LinkedBlockingQueue<TucsonOpWrapper> {
     ...
     *
     * @param action the TuCSoN action to be removed
     ...
     *
     * @param optype the operation type whose operations should be removed
     */
     ...
     * successful or not
     */
    public boolean removeOpById(final OperationIdentifier id) {
        for (TucsonOpWrapper tow : this) {
            if (tow.getOp().getId().equals(id)) {
                tow.setHasBeenRemoved(true);
                return this.remove(tow);
            }
        }
